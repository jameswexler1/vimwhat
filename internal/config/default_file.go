...
func DefaultFileContent(paths Paths) string {
	...
	b.WriteString("# Shift+Enter bindings only work when the terminal reports it distinctly; ctrl+j is the reliable composer newline fallback.\n\n")

	fmt.Fprintf(&b, "editor = %q\n", cfg.Editor)
	fmt.Fprintf(&b, "preview_backend = %q\n", cfg.PreviewBackend)
	fmt.Fprintf(&b, "emoji_mode = %q\n", cfg.EmojiMode)
	fmt.Fprintf(&b, "quick_reactions = %q\n", strings.Join(cfg.QuickReactions, " "))
	...
	fmt.Fprintf(&b, "indicator_notifications_muted = %q\n", cfg.IndicatorNotificationsMuted)
	...
	fmt.Fprintf(&b, "sticker_picker_command = %q\n", cfg.StickerPickerCommand)
	fmt.Fprintln(&b, "# auto probes installed viewers; explicit commands fail if unavailable.")
