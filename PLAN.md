...
- Local draft persistence, local outgoing message persistence, normal/visual message yanking, text clipboard integration, multi-format image clipboard paste/copy integration, attachment staging, message delete flow, and search routing by pane.
- Media backend detection and in-chat preview behavior with `sixel`, Unix `ueberzug++`, `chafa`, compact audio playback rows, plus native external open/save fallback paths. Windows auto preview now prefers Sixel in WezTerm when a Sixel renderer is installed, draws Sixel out-of-band instead of embedding image escape payloads in Bubble Tea text frames, and still supports explicit external/Chafa/none overrides.
...
- Paste image data from the system clipboard into the composer as the same single staged attachment used by normal media sends, and copy the focused downloaded image back to the system clipboard through configurable or auto-detected native platform commands.
