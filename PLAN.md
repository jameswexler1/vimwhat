...
- Large-history TUI guardrails: paired live startup renders cached chats immediately with non-blocking progress, gates protocol-backed mutations until reconnect catch-up is ready, waits for WhatsApp's offline-complete marker instead of treating a wall-clock timeout as success, keeps stalled catch-up active, tracks pending decrypt recoveries beyond the marker, publishes one final snapshot after the ordered local queue drains, summarizes replay notifications once, batches late recovered messages, tags offline-stream provenance plus pre-connection timestamps defensively, avoids historical refresh storms, commits message payloads with their messages, debounces ordinary live refreshes, stores chat-list preview text on chats, preserves focus across stale reloads, bounds rendered message windows, suppresses duplicate history requests, and keeps store/protocol callbacks asynchronous and deadline-bound.
