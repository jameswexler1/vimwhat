...
- Visible-first chat avatar sync and compact chat-list rendering, backed by WhatsApp profile-picture refreshes, cached local avatar files, `ueberzug++` pixel avatars when available, compact inline fallbacks, and initials fallback when image rendering is unavailable.
