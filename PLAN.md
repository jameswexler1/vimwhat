...
- Terminal/UI polish for real chat data: full/compat/auto emoji rendering, stable emoji fallback for terminals such as `st`, pywal-backed mode indicators with per-mode hex overrides, focused-pane borders that distinguish chat-list vs message-pane input focus, non-redundant mode prompts, configurable help/prompt key hints, search match counts in the status bar, and configurable cancellation/search-clear bindings.
