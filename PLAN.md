...
- Group-member mention autocomplete in the insert composer: typing `@` in group chats opens participant suggestions, typed text filters them with accent-aware matching, `Enter` or configurable `key_insert_mention_select_alt` completes the highlighted contact, and outgoing text/media captions carry WhatsApp `mentioned_jid` context.
