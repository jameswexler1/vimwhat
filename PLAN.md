...
- Recent sticker send support: WhatsApp recent-sticker history/app-state sync is persisted in SQLite, renderable WebP stickers are cached under the transient app directory at startup, `key_normal_pick_sticker` defaults to `leader t`, the picker command is configurable with an `nsxiv -t -o -p {files}` Linux default, and selected stickers are sent through a dedicated WhatsApp sticker message path. Lottie/TGS stickers are metadata-only for now.
