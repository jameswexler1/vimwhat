...
- Media backend detection and in-chat preview behavior with `sixel`, Unix `ueberzug++`, `chafa`, compact audio playback rows, plus native external open/save fallback paths. Linux auto preview now prefers `ueberzug++` pixel overlays before Sixel when both are available; Windows auto preview still prefers Sixel in WezTerm when a Sixel renderer is installed, falls back to inline `chafa` before external openers when `preview_backend = "auto"`, draws Sixel out-of-band instead of embedding image escape payloads in Bubble Tea text frames, and explicit backend config still wins on every platform.
...
- Media preview backends are platform-aware:
  Unix auto prefers `ueberzug++`, then `sixel`, then `chafa`, then the external opener; Windows auto prefers WezTerm Sixel when detected, then the native external opener, then `chafa`, while explicit backend config still wins.
