...
- Fetch WhatsApp app-state once after the live WhatsApp session connects, so favorite/recent stickers and chat mute/pin settings are populated without syncing on every picker open.
- Treat favorite-sticker file caching as best-effort: try WhatsApp media URLs before direct paths, persist metadata even when an individual sticker download is stale/unavailable, continue with other stickers, and report the first cache failure reason alongside unavailable counts.
