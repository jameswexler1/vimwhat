...
- Media backend detection and in-chat preview behavior with `sixel`, Unix `ueberzug++`, `chafa`, compact audio playback rows, plus native external open/save fallback paths. Windows auto preview prefers Sixel and then the native opener before `chafa`, with an explicit in-TUI prompt before using Chafa as a lower-resolution inline fallback for previews and chat avatars.
...
- Media preview backends, in priority order:
  ...
  Unix `ueberzug++` if available,
  `chafa` text/image fallback,
  external opener fallback via native platform opener/configured command.
