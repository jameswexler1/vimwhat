...
- Media backend detection and in-chat preview behavior with `sixel`, Unix `ueberzug++`, `chafa`, compact audio playback rows, plus native external open/save fallback paths. Windows auto preview prefers the native opener, keeps Chafa behind an explicit in-TUI lower-resolution inline fallback prompt for previews and chat avatars, and leaves Sixel available when explicitly configured or forced for capable terminals.
...
- Media preview backends are platform-aware:
  Unix auto prefers `sixel`, then `ueberzug++`, then `chafa`, then the external opener; Windows auto prefers the native external opener, keeps `chafa` as an accepted inline fallback, and uses `sixel` only when explicitly configured or forced.
