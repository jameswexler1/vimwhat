...
- Large-history TUI guardrails: paired live startup renders cached chats immediately with non-blocking progress, gates protocol-backed mutations until reconnect catch-up is ready, tracks pending decrypt recoveries beyond WhatsApp's offline-complete marker, publishes one final snapshot after a quiet period or bounded timeout, summarizes replay notifications once, batches late recovered messages, avoids historical refresh storms, debounces ordinary live refreshes, stores chat-list preview text on chats, preserves focus across stale reloads, bounds rendered message windows, suppresses duplicate history requests, and keeps store/protocol callbacks asynchronous and deadline-bound.
...
- Debounce live DB snapshot refreshes, consolidate reconnect catch-up behind non-blocking progress and a final snapshot barrier, wait for known message recoveries with a bounded timeout, summarize catch-up notifications once, and bound message render windows for chats with hundreds of loaded messages.
